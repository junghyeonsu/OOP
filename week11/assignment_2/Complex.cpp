...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* other = dynamic_cast<Integer*>(num); // 인티저로 형변환
		...
		other->set_val(other->real() + this->real(), other->imag());
		delete(this);
		return other;
	}
	else {
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		Complex* other = dynamic_cast<Complex*>(num); //
		other->set_val((this->real() * other->real()) - (this->imag() * other->imag()), (this->real() * other->imag()) + (this->imag() * other->real()));
		delete(this);
		return other;
	}
	else {
...
Number *Complex::div(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		Float* other = dynamic_cast<Float*>(num); // 인티저로 형변환
		if (other->val()== 0) {
			return nullptr;
		} else {
			this->set_val(this->real() / other->val(), this->imag());
